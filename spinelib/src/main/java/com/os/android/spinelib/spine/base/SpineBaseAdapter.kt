...
abstract class SpineBaseAdapter : ApplicationAdapter {
    ...
    // 设置缩放比例 以适应给定的空间大小
    // todo 暂时没找到动态获取的办法 先手动设置
    fun setViewScale(scaleX : Float, scaleY : Float){
...
//        Log.e("spinetest","viewHeight->$viewWidth  viewWidth->$viewHeight  originHeight->$originHeight  originWidth->$originWidth  " +
//                "${Gdx.graphics?.displayMode?.toString()}")

        mSkeleton = Skeleton(mSkeletonData)
     //   val scale :Float = AutoSizeUtil.getFullScreenWidth() / 1632f
        mSkeleton?.setScale(viewScaleX, viewScaleY)

//        Log.e("spinetest","${mSkeleton?.data?.width}   ${mSkeleton?.data?.height}")


        //设置骨架在父布局中的位置，需要Spine动画本身的原点在(0,-1)
        mSkeleton?.setPosition((Gdx.graphics.width / 2).toFloat(), (Gdx.graphics.height / 2).toFloat())
//        mSkeleton!!.setPosition(0f, 0f)
    ...
    private fun onResizeImpl(width: Int, height: Int) {
        mCamera.setToOrtho(false, width.toFloat(), height.toFloat())
        //Log.e("spine123","onResizeImpl  $width $height"  )
    }
    ...
    private fun onRenderImpl() {
        synchronized(SpineBaseAdapter::class.java) {
            if (hasInit) {
                try {
                    mAnimationState.update(spineBaseAnimView!!.graphics.deltaTime)
                }catch (e :Exception){
                    e.printStackTrace()
                }
     ...
     */
    @Synchronized
    fun create(activity: Activity,zOrderOnTop :Boolean = false): View? {
        spineBaseAnimView = SpineBaseAnimView(activity)
        spineBaseAnimView?.adapter = this
        spineBaseAnimView?.setZOrderOnTop(zOrderOnTop)
